...
// связь карты, названий и списков
	...
	jQuery( ".karta-bg-content-cities-ul li a" ).hover(
		function () {
			if (!jQuery('#f' + jQuery(this).attr('id').substr(1) + ':hover').length > 0) {
	     	jQuery('#f' + jQuery(this).attr('id').substr(1)).mouseover();
	     }
		}, function () {
	     	jQuery('#f' + jQuery(this).attr('id').substr(1)).mouseout();
		}
	);


	jQuery( ".area-maping" ).hover(
	  function() {
	     	jQuery('.name' + jQuery(this).attr('id').substr(1)).css('display','block');
	  	jQuery('#n' + jQuery(this).attr('id').substr(1)).addClass('active-class');
	  }, function() {
	  	jQuery('#n' + jQuery(this).attr('id').substr(1)).removeClass('active-class');

	  	if (!jQuery('.name' + jQuery(this).attr('id').substr(1) + ':hover').length > 0) {
	     	jQuery('.name' + jQuery(this).attr('id').substr(1)).css('display','none');
	  	}
	...
	jQuery( ".map-name" ).hover(
	  function() {
	  	jQuery('#n' + jQuery(this).attr('class').substr(13)).addClass('active-class');
	  },
	  function() {
  		if (!jQuery('#f' + jQuery(this).attr('class').substr(13) + ':hover').length > 0) {
	     	jQuery(this).css('display','none');
	  		jQuery('#n' + $(this).attr('class').substr(13)).removeClass('active-class');
  		}
	  }
	);
	...
	var array_form = {
		...
		{"name": 'Киев', "phon": " +38-96-888-76-88",  "phon2": "", "skyppe": "kiev@pv-student.cz", "adressee":"МЕНЕДЖЕР ПРИЕДЕТ К ВАМ", "image" : "http://pv-student.cz/wp-content/themes/Mrabos/images/city/kiev.png", "hourr" : "3"},
		{"name": 'Мурманск', "phon": "+7-964-680-16-57", "phon2": "8(152) 601-657", "skyppe": "murmansk@pv-student.cz", "adressee": "МЕНЕДЖЕР ПРИЕДЕТ К ВАМ", "image" : "http://pv-student.cz/wp-content/themes/Mrabos/images/city/murmansk.png", "hourr" : "3"},
		...
		{"name": 'Баку', "phon": "+994-50-754-76-90",  "phon2": "", "skyppe": "baku@pv-student.cz", "adressee": "МЕНЕДЖЕР ПРИЕДЕТ К ВАМ", "image" : "http://pv-student.cz/wp-content/themes/Mrabos/images/city/baku.png", "hourr" : "4"},
		...
		{"name": 'Астана', "phon": "+7-771-463-55-51",  "phon2": "", "skyppe": "astana@pv-student.cz", "adressee": "МЕНЕДЖЕР ПРИЕДЕТ К ВАМ", "image" : "http://pv-student.cz/wp-content/themes/Mrabos/images/city/astana.png", "hourr" : "6"},
	...
	function sendfromform(id) {
		jQuery('#place').attr('value', array_form.place[id].name);
		// $('.overlay').css('display','block');
		// $('.form-map').css('display','block');
		jQuery('.rayon').empty();
		jQuery('.rayon').append(array_form.place[id].name);

		var phon = '';
		jQuery('.phonne').empty();
		jQuery('.phonne').append(array_form.place[id].phon);
		var phon2 = '';
		jQuery('.phonne2').empty();
		jQuery('.phonne2').append(array_form.place[id].phon2);
		var skyppe = '';
		jQuery('.skyperr').empty();
		jQuery('.skyperr').append(array_form.place[id].skyppe);
		var adressee = '';
		jQuery('.adresse').empty();
		jQuery('.adresse').append(array_form.place[id].adressee);

		var image = '';
		jQuery('.bg-image').empty();
		jQuery('.bg-image').attr("src",array_form.place[id].image);
		...
		function startTimeCity()   {
			...
			jQuery('.hourss').empty();
			jQuery('.hourss').append(finishTime);	
	...
	jQuery(document).on('click', '.map-name', function(){
	  sendfromform(jQuery(this).attr('class').substr(13));	  
	  return false
	});

	jQuery(document).on('click', '.area-maping', function(){
	  sendfromform(jQuery(this).attr('id').substr(1));
	  return false
	});



	jQuery(document).on('click', '.karta-bg-content-cities-ul li a', function (){
	  sendfromform(jQuery(this).attr('id').substr(1));
	  // function ssga2() {
